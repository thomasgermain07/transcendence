...
export class FriendshipsController {
	...
	async create(
		...
		const friendship: Friendship = await this.friendships_svc.findOneOr(
			user,
			target,
		);

		if (!friendship) return this.friendships_svc.create(user, target);

		if (friendship.user.id === user.id) return friendship;
