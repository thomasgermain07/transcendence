...
export class PermissionsController {
  ...
  async get(
    ...
    return (await this.permissions_svc.find({ room: room, type: type })).filter(
      (permission) =>
        !permission.expired_at || permission.expired_at.getTime() >= Date.now(),
    )
  }
  ...
  async create(
    ...
    if (!target || !room) throw new NotFoundException('Room or User not found.')

    if (!(await this.chat_svc.isModerator(user, room)))
      throw new ForbiddenException('You are not moderator for this room.')

    if (
      (await this.chat_svc.isModerator(target, room)) &&
      !(await this.chat_svc.isLeader(user, room))
    )
    ...
    const permission: Permission = await this.permissions_svc.create(
      target,
      room,
      create_dto,
    )

    this.chat_gateway.setPermission(permission)
  ...
  async remove(
    ...
    if (
      ...
      throw new ForbiddenException(
        'You can not remove role for other moderator.',
      )
    ...
    this.chat_gateway.removePermission(permission)
    this.permissions_svc.remove(permission)
  }
