...
export class RoomsSubscriber implements EntitySubscriberInterface<Room> {
	...
	private async defenseAchievements(
		room: Room,
		player: Player,
	): Promise<Player> {
		if (room.players[0].winner) {
			...
			if (room.players[1].score == 0) {
				await this.usersService.updateAchievements(
					player.user,
					AchievementsName.DEFENSE_MASTER,
				);
			}
		} else {
	...
	private async difficultyAchievements(player: Player): Promise<void> {
		...
		if (room.option.difficulty == DifficultyLevel.MEDIUM) {
			await this.usersService.updateAchievements(
				player.user,
				AchievementsName.MIDDLE_PLAYER,
			);
		} else if (room.option.difficulty == DifficultyLevel.HARD) {
			await this.usersService.updateAchievements(
				player.user,
				AchievementsName.HARD_MASTER,
			);
		}
	}

	private async winneAcchivements(player: Player): Promise<void> {
		const rooms: Room[] = await this.roomsService.findAllWinsByUser(
			player.user,
		);
		switch (rooms.length) {
			case 10:
			...
			case 200:
				await this.usersService.updateAchievements(
					player.user,
					AchievementsName.TWO_HUNDRED_WINS,
				);
				break;
		}
	}
	...
	private async allTerrainAchievements(player: Player): Promise<void> {
		const achieved = await this.roomsService.findWinsByUserInMapDuel(
			player.user,
		);
		if (achieved) {
			await this.usersService.updateAchievements(
				player.user,
				AchievementsName.ALL_TERRAIN,
			);
		}
	}
