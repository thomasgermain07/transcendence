...
export class Paddle implements IMapPaddleState {
	...
	paddle_move(ball: IBallState): void {
		...
		if (this.move == Direction.UP) {
			...
			if (
				topX > this.x &&
				botX < this.x + HEIGHT / 80 &&
				topY + ball.yspeed > upy &&
				botY + ball.yspeed < upy + WIDTH / this.height
			) {
			...
			if (
				topX > this.x &&
				botX < this.x + HEIGHT / 80 &&
				topY + ball.yspeed > downy &&
				botY + ball.yspeed < downy + WIDTH / this.height
			) {
				this.y = topY + ball.yspeed;
			}
			this.y =
				downy + WIDTH / this.height >= WIDTH
