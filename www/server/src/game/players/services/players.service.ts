...
export class PlayersService {
	...
	public async checkIfInGame(user: User): Promise<Player> {
	...
	public async getInactive(): Promise<Player[]> {
		const inactivePlayers = await this.playersRepository
	...
	public async getMatchHistoryByUser(userId: number): Promise<Player[]> {
		const userMatchHistory = await this.playersRepository
			.createQueryBuilder('player')
	...
	private async addPlayer(room: Room, user: User): Promise<Player> {
