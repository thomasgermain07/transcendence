...
export default {
  setup() {
    ...
    let { users, fetchUsers, oldOffset, offset } = getFetchUsers(status, loading, search)

    const handleScroll = () => {
      if (!loading.value && (window.innerHeight + document.documentElement.scrollTop) >= document.documentElement.offsetHeight) {
