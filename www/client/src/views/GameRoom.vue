...
<script lang="ts">
import { defineComponent, computed, onMounted, onUnmounted, onBeforeMount } from 'vue'
import { ref } from 'vue';
  ...
  setup() {
    ...
    const checkReady = (room: Room): void => {
      if (room.players.every((player) => player.isReady === true)) {
        ...
        gameRoomsSocket.emit('init', {socketRoomName: roomName, room: room, players: room.players});
    ...
    gameRoomsSocket.on('begin', (data) => {
      ...
      if ( option.map != MapType.MAP1 ) {
		...
		function update(x: number, y: number) {
			...
			const update_data = {x: x * (canvas.width / 600), y: y * (canvas.height / 400)};
			return update_data;
		}
      ...
      if (event.key === "ArrowUp") {
				gameRoomsSocket.emit('move', { move: "up", user_id: currentUser.id, room: roomName });
				console.log("KEY UP");
			}
			else if (event.key === "ArrowDown") {
				gameRoomsSocket.emit('move', { move: "down",  user_id: currentUser.id, room: roomName });
      ...
      event.preventDefault();
		...
		function keyup(event: KeyboardEvent) {
			if (event.key === "ArrowUp") {
				gameRoomsSocket.emit('move', { move: "not", user_id: currentUser.id, room: roomName});
				console.log("KEY UP");
			}
			else if (event.key === "ArrowDown") {
				gameRoomsSocket.emit('move', { move: "not",  user_id: currentUser.id, room: roomName});
    ...
    function countdown() {
      ...
      if (option.count > 0)
        ctx.fillText(option.count.toString(), canvas.width / 2  , canvas.height / 2);
		...
		function drawMidleLine() {
			...
			ctx.moveTo(canvas.width * (canvas.width / 1088) - 600/80, canvas.height * (canvas.height / 544) - 80); // bottom left
			ctx.lineTo(canvas.width * (canvas.width / 1088) + 600/80, canvas.height * (canvas.height / 544) - 80); // bottom right
			ctx.lineTo(canvas.width * (canvas.width / 1088) + 600/80, canvas.height * (canvas.height / 544) + 80); // top right
			ctx.lineTo(canvas.width * (canvas.width / 1088) - 600/80, canvas.height * (canvas.height / 544) + 80); // top left
			ctx.fill();
      ctx.closePath();
		}
		...
		function drawPaddle() {
			...
			ctx.fillRect(data.x, data.y, canvas.width/80, canvas.height/player_left.paddle.height);
			data = update(player_right.paddle.x, player_right.paddle.y);
			ctx.fillRect( data.x, data.y, canvas.width/80, canvas.height/player_right.paddle.height);     
			ctx.fill();
			ctx.closePath();
		...
		function drawScore() {
			...
			ctx.textAlign = 'left';
			ctx.fillText("Score1 : ", canvas.width/2 - canvas.width/2.5, canvas.width/10);
			ctx.fillText(player_left.score.toString(), canvas.width/2 - canvas.width/8, canvas.width/10);

			ctx.fillText("Score2 :", canvas.width/2 + canvas.width/10, canvas.width/10);
			ctx.fillText(player_right.score.toString(), canvas.width/2 + canvas.width/2.5, canvas.width/10);
		}
